(function () {
  ...
  function bindForms() {
    ...
    bindAddTitleEditor(addTitleForm);
  ...
  function bindAddTitleEditor(form) {
    if (!form || form.dataset.bound === "true") return;
    form.dataset.bound = "true";

    const populate = () => {
      const translators = Object.values(state.users).filter((user) =>
        Array.isArray(user.roles) && user.roles.includes("Tłumacz")
      );

      form.querySelector("#newTitleTranslatorChecks").innerHTML = translators.map((user) => `
        <label class="pretty-check">
          <input type="checkbox" value="${escapeAttr(user.id)}" data-new-translator>
          <span class="pretty-check-box"></span>
          <span>${escapeHtml(user.displayName)}</span>
        </label>
      `).join("") || `<span class="editor-help">Brak osób z rolą „Tłumacz”.</span>`;

      const people = Object.values(state.users).filter((user) =>
        Array.isArray(user.roles) &&
        ["Właściciel","Współwłaściciel","Tłumacz","Pomocnik tłumacza"].some((role) => user.roles.includes(role))
      );

      form.querySelector("#newTitleEditorPickers").innerHTML = ["Cleaner","Korektor","Typesetter"].map((role) => `
        <details class="editor-picker">
          <summary>${role} <span data-new-editor-summary="${role}">Brak</span></summary>
          <div class="editor-picker-list">
            <label class="pretty-check">
              <input type="checkbox" data-new-editor-none="${role}" checked>
              <span class="pretty-check-box"></span><span>Brak</span>
            </label>
            ${people.map((user) => `
              <label class="pretty-check">
                <input type="checkbox" data-new-editor="${role}" value="${escapeAttr(user.id)}">
                <span class="pretty-check-box"></span>
                <span>${escapeHtml(user.displayName)}</span>
              </label>
            `).join("")}
    ...
    form.querySelectorAll('input[name="cover"], input[name="banner"]').forEach((input) => {
      input.addEventListener("change", async () => {
        ...
        try {
          ...
          if (preview) { preview.src = data; preview.classList.add("is-visible"); }
    ...
    form.addEventListener("click", (event) => {
      const add = event.target.closest("[data-add-new-person]");
      if (add) {
        const list = form.querySelector(`[data-new-people-list="${CSS.escape(add.dataset.addNewPerson)}"]`);
    ...
    form.addEventListener("change", (event) => {
      ...
      if (input.matches("[data-new-editor]")) {
        const role = input.dataset.newEditor;
        form.querySelectorAll(`[data-new-editor="${CSS.escape(role)}"]`).forEach((other) => {
          if (other !== input) other.checked = false;
        });
        form.querySelector(`[data-new-editor-none="${CSS.escape(role)}"]`).checked = false;
        const selected = form.querySelector(`[data-new-editor="${CSS.escape(role)}"]:checked`);
        form.querySelector(`[data-new-editor-summary="${CSS.escape(role)}"]`).textContent = selected ? displayUserName(selected.value) : "Brak";
      }

      if (input.matches("[data-new-editor-none]")) {
        const role = input.dataset.newEditorNone;
        if (input.checked) form.querySelectorAll(`[data-new-editor="${CSS.escape(role)}"]`).forEach((other) => other.checked = false);
        form.querySelector(`[data-new-editor-summary="${CSS.escape(role)}"]`).textContent = "Brak";
      }
    });
  }
  ...
  function renderAll() {
    ...
    renderTranslations();
    populatePeopleSelects();
    bindAddTitleEditor(document.querySelector("#addTitleForm"));
  ...
  async function handleAddTitle(event) {
    ...
    const newTitle = {
      ...
      authors: document.querySelector("#authorUnknown").checked
        ? [{ name: "Nieznany", url: "", status: "unknown" }]
        : Array.from(document.querySelectorAll('[data-new-people-list="authors"] [data-person-row]')).map((row) => ({
            name: row.querySelector("[data-person-name]")?.value.trim() || "",
            url: row.querySelector("[data-person-url]")?.value.trim() || "",
            status: row.querySelector("[data-person-status]")?.value || "profile"
          })).filter((person) => person.name),
      artists: document.querySelector("#artistUnknown").checked
        ? [{ name: "Nieznany", url: "", status: "unknown" }]
        : Array.from(document.querySelectorAll('[data-new-people-list="artists"] [data-person-row]')).map((row) => ({
            name: row.querySelector("[data-person-name]")?.value.trim() || "",
            url: row.querySelector("[data-person-url]")?.value.trim() || "",
            status: row.querySelector("[data-person-status]")?.value || "profile"
          })).filter((person) => person.name),
      translators: Array.from(document.querySelectorAll("[data-new-translator]:checked")).map((input) => input.value),
      editors: ["Cleaner","Korektor","Typesetter"].map((role) => {
        const input = document.querySelector(`[data-new-editor="${CSS.escape(role)}"]:checked`);
        return input ? { role, userId: input.value, name: displayUserName(input.value) } : { role, userId: null, name: "Brak" };
      }),
      chapters: []
    };
