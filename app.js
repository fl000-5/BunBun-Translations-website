(function () {
  ...
  function bindTeam() {
    ...
    document.querySelector("[data-team-prev]").addEventListener("click", () => {
      document.querySelector("#teamTrack").scrollBy({ left: -360, behavior: "smooth" });
    });
  ...
  function renderLatest() {
    ...
    grid.innerHTML = state.translations.slice(0, 8).map((title) => {
      const lastChapters = title.chapters.slice(-3).reverse().map((chapter) => {
        return `<p>${escapeHtml(chapter.number)} · ${escapeHtml(formatDate(chapter.date))}</p>`;
      }).join("");
      return `
  ...
  function renderTeam() {
    ...
    adminList.querySelectorAll("[data-edit-team]").forEach((button) => {
      button.addEventListener("click", () => {
        const member = state.team.find((item) => item.id === button.dataset.editTeam);
        if (!member) return;
    ...
    adminList.querySelectorAll("[data-delete-team]").forEach((button) => {
      button.addEventListener("click", () => {
        state.team = state.team.filter((item) => item.id !== button.dataset.deleteTeam);
        persistState();
        renderTeam();
      });
    });
  ...
  function startTeamLoop() {
    clearInterval(teamLoopTimer);
    const track = document.querySelector("#teamTrack");
    if (!track || track.children.length < 2) return;
    teamLoopTimer = window.setInterval(() => {
      if (document.hidden || !track.closest(".view.is-active")) return;
      const firstCard = track.querySelector(".team-card");
      const gap = 24;
      const step = firstCard ? firstCard.offsetWidth + gap : 360;
