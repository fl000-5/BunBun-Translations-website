(function () {
  ...
  function renderTeam() {
    ...
    if (state.teamCarouselIndex >= members.length) state.teamCarouselIndex = 0;
    ...
    requestAnimationFrame(() => {
      ...
      track.scrollLeft = (members.length + state.teamCarouselIndex) * step;
    });
  ...
  function moveTeamCarousel(direction) {
    ...
    if (!metrics || metrics.membersCount <= 1) return;

    const { track, membersCount } = metrics;
    const current = state.teamCarouselIndex || 0;
    const next = (current + direction + membersCount) % membersCount;
    const cards = track.querySelectorAll(".team-card");
    const middleIndex = membersCount + next;
    const targetCard = cards[middleIndex];

    if (!targetCard) return;

    state.teamCarouselIndex = next;

    // The track contains three copies of the same team. We always move to
    // the corresponding card in the middle copy, so the sequence is truly
    // circular: ... Mika -> Flooo -> Katsumi ... without autoplay.
